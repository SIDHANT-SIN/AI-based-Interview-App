...
/* ── INNER ROOM ─────────────────────────────────────────────── */
function HRInterviewRoom({ roomName }: { roomName: string }) {
  const navigate = useNavigate();
      ...
      <div className="relative flex-1 w-full flex items-center justify-center p-4 sm:p-8 min-h-0 z-0">
        <div className="relative w-full aspect-video max-w-[min(calc((100vh-180px)*16/9),100%)] rounded-2xl overflow-hidden bg-white/[0.02] border border-white/10 shadow-[0_12px_40px_rgba(0,0,0,0.4)] backdrop-blur-sm">
          <div className="absolute inset-0 flex items-stretch justify-center">
            ...
            {aiAgent && (
              <div className="flex-1 flex flex-col items-center justify-center bg-[#0b0e1a]/50 border-l border-white/10">
...
/* ── LOCAL VIDEO FEED ───────────────────────────────────────── */
function LocalVideoFeed({ isCameraOff }: { isCameraOff: boolean }) {
  ...
  if (isCameraOff || !localTrack) {
    return (
      <div className="flex-1 h-full flex flex-col items-center justify-center gap-3 bg-white/[0.02]">
